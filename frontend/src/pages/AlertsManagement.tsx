  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-text-bold tracking-tight">
            Active Monitors
          </h1>
          <p className="text-text-muted mt-1 font-bold text-sm">
            Manage your intelligent stock alerts and conditions.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-white/[0.02] border border-white/5 rounded-[2rem] border-dashed">
          ...
          <h2 className="text-xl font-black text-text-bold">
            No Alerts Configured
          </h2>
          <p className="text-text-muted mt-2 max-w-xs mx-auto text-sm font-medium">
            Go to any stock detail page and click the bell icon to set up
            ...
            {alerts.map((alert) => (
              <motion.div
                ...
                className={`group relative overflow-hidden bg-[#121212] border transition-all duration-300 rounded-[1.5rem] p-6 ${
                  alert.is_active
                    ? "border-white/10 hover:border-accent/30 shadow-xl"
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-6">
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-xl font-black text-text-bold tracking-tight uppercase group-hover/item:text-accent transition-colors">
                          {alert.symbol}
                        </h3>
                        {!alert.is_active && (
                          <span className="text-[10px] font-black uppercase tracking-widest bg-white/5 text-text-muted px-2 py-0.5 rounded-full border border-white/10">
                            Paused
                          </span>
                        )}
                        {alert.is_triggered && (
                          <span className="text-[10px] font-black uppercase tracking-widest bg-success/10 text-success px-2 py-0.5 rounded-full border border-success/20">
                            Triggered
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex-1 min-w-[300px] flex flex-wrap gap-2">
                    {Array.isArray(alert.condition) &&
                      alert.condition.map((c: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-2 rounded-xl"
                        >
                          {getIndicatorIcon(c.indicator)}
                          <span className="text-xs font-black text-text-bold uppercase tracking-tight">
                            {c.indicator}
                          </span>
                          <span className="text-xs font-bold text-accent">
                            {c.operator}
                          </span>
                          <span className="text-xs font-black text-text-bold">
                            {c.value}
                          </span>
                        </div>
                      ))}
